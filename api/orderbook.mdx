...
Retrieves all orders that were settled in a specific transaction.

**Endpoint**: `GET /api/v1/orders/tx/{txHash}`
...
Returns the current batch auction that solvers should be solving. Includes solvable orders, block number, and token prices for objective value computation.
...
Returns the total surplus (price improvement) earned by a user across all their trades.

**Endpoint**: `GET /api/v1/users/{address}/total_surplus`
...
<ResponseField name="totalSurplus" type="string">
  Total surplus earned in the native token (e.g., ETH)
</ResponseField>

<ResponseField name="numberOfTrades" type="integer">
